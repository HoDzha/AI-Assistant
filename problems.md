...
**Решение:** PDF-генерация перенесена на бэкенд через `fpdf2`. Эндпоинт `POST /api/export-pdf` возвращает готовый PDF с корректной поддержкой кириллицы (шрифт Arial/System). Фронтенд получает файл как `Blob` и инициирует скачивание.
...
**Серьёзность:** 🟡 Medium → ✅ **Неактуально**

**Решение:** jsPDF удалён из проекта. PDF-генерация теперь выполняется на бэкенде.
